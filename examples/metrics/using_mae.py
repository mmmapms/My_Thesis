...
forecast_aux = pd.read_csv('C:/Users/mmmap/Desktop/Thesis/Results/Values_Forecast_NP_DNN_LEAR.csv', index_col=0)
forecast = forecast_aux.dropna()
...
_, df_test = read_data(path='C:/Users/mmmap/epftoolbox/datasets/', dataset='NP', begin_test_date='27/12/2016', 
...
a=MAE(p_pred=fc_DNN_ensemble, p_real=real_price)

# Evaluating MAE when real price and forecasts are both numpy arrays
b=MAE(p_pred=fc_DNN_ensemble.values, p_real=real_price.values)

# Evaluating MAE when input values are of shape (ndays, n_prices/day) 
# instead of shape (nprices, 1)
# Dataframes
c=MAE(p_pred=fc_DNN_ensemble_2D, p_real=real_price_2D)
# Numpy arrays
d=MAE(p_pred=fc_DNN_ensemble_2D.values, p_real=real_price_2D.values)

# Evaluating MAE when input values are of shape (nprices,) 
# instead of shape (nprices, 1)
# Pandas Series
e=MAE(p_pred=fc_DNN_ensemble.loc[:, 'DNN'], 
    p_real=real_price.loc[:, 'Price'])
# Numpy arrays
f=MAE(p_pred=fc_DNN_ensemble.values.squeeze(), 
